...
update_log()
{
  ...
  echo "Update did to update_elastos-did-v0.3.1">>$SCRIPT_PATH/update.log
  echo "">>$SCRIPT_PATH/update.log
...
update_node()
{
  ...
  wget https://download.elastos.org/elastos-did/elastos-did-v0.3.1/SHA256SUMS
  shasum -c SHA256SUMS
  tar xf elastos-did-v0.3.1-linux-x86_64.tgz
  ~/node/node.sh did stop
  cp  elastos-did-v0.3.1/did ~/node/did/
  rm -r ~/node/did/elastos_did/data/spv/
  ~/node/node.sh did start
  rm -r elastos-did-v0.3.1-linux-x86_64.tgz elastos-did-v0.3.1 SHA256SUMS
}
